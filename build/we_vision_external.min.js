...
/***/ function(module, exports, __webpack_require__) {
	...
	WeVisionWidget = (function() {
	  var API_URL, config, lastIdSearch;
	  ...
	  WeVisionWidget.prototype.iframe = null;

	  function WeVisionWidget() {
	    var data, product_id, script, scriptSrc;
	    ...
	    this.getProducts(product_id, function(err, response) {
	      var html, we_vision_internal_src, widget;
	      if (err) {
	        return;
	      }
	      this.iframe = document.createElement('iframe');
	      this.iframe.setAttribute('width', '100%');
	      this.iframe.setAttribute('id', 'we-vision-iframe');
	      ...
	      widget.appendChild(this.iframe);
	      iFrameResize({
	        checkOrigin: false
	      }, this.iframe);
	      if (config.mode === 'debug') {
	        ...
	        html = '<!DOCTYPE html> <html> <head> <script> var config = ' + config + '; var products = ' + response + '; var product_id = "' + product_id + '"; </script> <base target="_blank"/> </head> <body> <section id="widget"></section> <section id="preview" class="magnifier-preview" style="width: 200px; height: 184px;"></section> <script src="' + scriptSrc + '/vendor/requirejs/require.js"></script> <script src="' + scriptSrc + '/vendor/iframe-resizer/src/iframeResizer.contentWindow.js"></script> <script src="' + scriptSrc + '/we_vision_internal.js"></script> </body> </html>';
	      } else if (config.mode === 'production' || !config.mode) {
	        response = JSON.stringify(response);
	        config = JSON.stringify(config);
	        we_vision_internal_src = '/build/we_vision_internal.min.js';
	        html = '<!DOCTYPE html> <html> <head> <script> var config = ' + config + '; var products = ' + response + '; var product_id = "' + product_id + '"; </script> <base target="_blank"/> </head> <body> <section id="widget"></section> <section id="preview" class="magnifier-preview" style="width: 200px; height: 184px;"></section> <script src="' + we_vision_internal_src + '"></script> </body> </html>';
	      }
	      this.iframe.contentWindow.document.open();
	      this.iframe.contentWindow.document.write(html);
	      return this.iframe.contentWindow.document.close();
	    });
	  }

	  WeVisionWidget.prototype.postMessage = function(msg) {
	    return this.iframe.contentWindow.postMessage(msg, "*");
	  };

	  WeVisionWidget.prototype.getProductId = function() {
	    var id, idCont;
	    idCont = document.querySelector(config.productIdContainer);
	    return id = idCont.innerHTML;
	  };

	  WeVisionWidget.prototype.getProducts = function(id, callback) {
	    var data, req;
	    if (id !== this.lastIdSearch) {
